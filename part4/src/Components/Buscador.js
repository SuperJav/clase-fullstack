...
const Buscador=(props)=>{
    ...
    const respuesta=()=>{
        ...
        casa= estado.filter(index=> index.name.localeCompare(busqueda, undefined, { sensitivity: 'base' }) ===0)

        // Metodo que retorna una lista de los nombre encontrado
         resultado=()=>{return casa.map(index=><h4 key={index.id}>{`${index.name} tel: ${index.number}`}</h4>)}
        return resultado()
    }
    useEffect(()=>
